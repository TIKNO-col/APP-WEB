...
export default function Auth() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      setLoading(true)
      const response = await fetch('https://erp-tikno.onrender.com/api/auth/login/', {
        method: 'POST',
        headers: {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      setLoading(true)
      const response = await fetch('https://erp-tikno.onrender.com/api/auth/registro/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div className="relative flex min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      {/* Fondo con degradado responsivo */}
      ...
      {/* Lado derecho - Decoración - Solo visible en pantallas grandes */}
      <motion.div 
